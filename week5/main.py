...
def main():
    ...
    for f in sorted(glob.glob(qs_l)):
        ...
        # Iterate the masks (1 or 2 or 3 according to the images)
        ...
        for m in range(length):
            ...
            text = cv.bitwise_not(mask[m])

            # If a text mask is all zeros: all ones
            if text.any()==0:
                print("ALL ZERO")
                height, width = np.shape(text)
                text = np.ones((height,width),np.uint8)
    ...
    if QUERY_SET == 'qsd1_w2' or QUERY_SET == 'qsd2_w2' or QUERY_SET == 'qsd1_w3' or QUERY_SET == 'qsd2_w3' or QUERY_SET == 'qsd1_w4' or QUERY_SET == 'qsd1_w5':
        ...
        if QUERY_SET == 'qsd1_w4' or QUERY_SET == 'qs_guasa' or QUERY_SET == 'qsd1_w5':
