...
def extract_features(img, mask, nbins, divs):
...
# Extracts feature vector from image. Returns a flat list consisting of the obtained features
    ...
    for i in range(0,divs):
        for j in range(0,divs):
            ...
            subimg = img[i*round(img.shape[0]/divs):(i+1)*round(img.shape[0]/divs)-1, 
                         j*round(img.shape[1]/divs):(j+1)*round(img.shape[1]/divs)-1]
            if mask is not None :
                submask = mask[i*round(img.shape[0]/divs):(i+1)*round(img.shape[0]/divs)-1, 
                            j*round(img.shape[1]/divs):(j+1)*round(img.shape[1]/divs)-1]
            else :
                submask = None
